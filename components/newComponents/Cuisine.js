...
const Cuisine =({cui,evnt,number,active})=>(
    <TouchableOpacity style={styles.container}
                      onPress={evnt}>
        <Image  source={cuisineImages[lib.lowercaseAndJoin(cui)]}
                style={styles.imageStyle}/>
        <Text numberOfLines={1} style={[styles.name,(number===active)?{color:colors.a}:{color:'black'}]}>{lib.format(cui)}</Text>
    </TouchableOpacity>
)
