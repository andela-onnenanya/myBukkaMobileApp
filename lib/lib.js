...
export default {
  ...
  addcard(cardNumber, ccv, expirationMonth, expirationYear) {
    ...
    storage
      ...
      .then(val => {
        storage
          .dispatch(time_to_reauthenticate(this.reauth(val.auth, val.email)))
          .catch(e => {
            e.response ? alert(e.response.data.msg) : console.log(e.message);
          });
      })
      .catch(e => {
  ...
  clearall: () => storage.dispatch(clearorders()),
  updateuser: newProfile => {
    const uid = storage.getState().user.user.uid;
    const url = ajx.edit_user + uid;
    const {
      email,
      mobile,
      first_name,
      last_name
    } = storage.getState().user.user;
